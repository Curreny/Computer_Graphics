...
var axis = 0;
var theta = [0, 0, 0];
var translate = [0.0, 0.0, 0.0]; // 新增平移数组
...
window.onload = function initCube() {
    ...
    document.getElementById("xSlider").oninput = function (event) {
        translate[0] = parseFloat(event.target.value);
        gl.uniform3fv(translateLoc, translate);
    };

    document.getElementById("ySlider").oninput = function (event) {
        translate[1] = parseFloat(event.target.value);
        gl.uniform3fv(translateLoc, translate);
    };

    document.getElementById("zSlider").oninput = function (event) {
        translate[2] = parseFloat(event.target.value);
        gl.uniform3fv(translateLoc, translate);
    };
