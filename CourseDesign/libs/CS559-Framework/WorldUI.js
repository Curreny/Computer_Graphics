...
export class WorldUI {
     ...
     */
    constructor(world, width = 700, where = undefined, grading = true) {
        const self = this;
        this.world = world;
        ...
        const headTitle = InputHelpers.makeHead("Interactive controls", this.div, { tight: true });
        ...
        this.selectionChkList.className = "centered-flex-div";
        /**@type HTMLInputElement */
        this.chkSolo = InputHelpers.makeCheckbox(
            "chkSolo",
            this.selectionChkList,
            "View individual object perspectives"
        );
        ...
        if (grading) {
            ...
            // emoteButton("Grader", "https://graphics.cs.wisc.edu/test/grading.js", world, this.selectionChkList);
        }
        //
        //

        // 添加选择天空盒的控件
        InputHelpers.makeSpan("Skybox:", this.div);
        this.selectSkybox = InputHelpers.makeSelect(
            ["skybox1", "skybox2", "skybox3"],
            this.div
        );
        this.selectSkybox.onchange = function () {
            const selectedSkybox = self.selectSkybox.value;
            const loader = new T.CubeTextureLoader();
            loader.setPath(`../textures/${selectedSkybox}/`);
            const textureCube = loader.load([
                'posx.jpg', 'negx.jpg',
                'posy.jpg', 'negy.jpg',
                'posz.jpg', 'negz.jpg'
            ]);
            _world.scene.background = textureCube;
        };

        // 触发一次 onchange 以加载默认天空盒
        this.selectSkybox.onchange(null);

        // 添加提示词
        InputHelpers.makeSpan("View Mode:", this.div);
